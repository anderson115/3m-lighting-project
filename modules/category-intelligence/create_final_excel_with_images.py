#!/usr/bin/env python3
"""
Create final Excel with all data including extracted images.
"""
import json
...
print("="*70)
print("CREATING FINAL DELIVERABLE WITH IMAGES")
print("="*70)
print()

# Load enhanced data with images
with open('data/retailers/all_products_enhanced_with_images.json', 'r') as f:
...
print(f"  • Brands: {len([p for p in products if p.get('brand') or p.get('shop')])}/{len(products)}")
print(f"  • Product URLs: {len([p for p in products if p.get('url')])}/{len(products)}")
print(f"  • Prices: {len([p for p in products if p.get('price')])}/{len(products)}")
print(f"  • Ratings: {len([p for p in products if p.get('rating')])}/{len(products)}")
print()
