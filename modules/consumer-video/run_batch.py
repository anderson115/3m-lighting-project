...
from process_optimized import process_video
...
def process_batch(batch_number, videos):
    ...
    print(f"Time: {batch_time/60:.1f} minutes ({batch_time:.1f}s)")
    print(f"Average: {batch_time/len(videos):.1f}s per video")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    print(f"Created {len(batches)} batches of ~8 videos each")

    # Process only the batch specified (or first batch by default)
