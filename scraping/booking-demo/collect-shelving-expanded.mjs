...
// EXPANDED shelving searches - many more queries to reach 18-20% coverage
const RETAILERS = [
  {
    name: "Lowes",
    searches: [
  ...
  try {
    ...
    const products = await page.evaluate((urlPattern, maxItems) => {
      ...
      for (let i = 0; i < Math.min(uniqueLinks.length, maxItems); i++) {
        ...
        // Filter: Must contain shelving/shelf/rack terms
        ...
        // Skip hooks, single brackets, wall-mount hooks
        if (titleLower.includes('hook') ||
            titleLower.includes('hanger') ||
            titleLower.includes('bracket only') ||
            (titleLower.includes('bracket') && !titleLower.includes('unit'))) {
          continue;
        }
    ...
    console.log(`   ✓ Found ${products.length} shelving products`);
...
async function main() {
  ...
  console.log("EXPANDED SHELVING COLLECTION - Target: +600-800 products");
  console.log("=".repeat(70));
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log(`EXPANDED SHELVING COLLECTION COMPLETE`);
  console.log(`${"=".repeat(70)}`);
  console.log(`Total unique shelving: ${allProducts.length}`);
  ...
  if (allProducts.length > 0) {
    ...
    fs.writeFileSync('shelving_expanded_collection.json', JSON.stringify(output, null, 2));
    console.log("\n✓ Saved to shelving_expanded_collection.json");
...
main().then(count => {
  console.log(`\n✅ Expanded collection complete: ${count} unique shelving products`);
