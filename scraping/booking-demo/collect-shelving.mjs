...
// Shelving search queries - focused on non-hook shelving products
const RETAILERS = [
  {
    name: "Lowes",
    searches: [
      "garage shelving unit",
      "wire shelving garage",
      "metal shelving",
      "heavy duty shelving",
...
async function main() {
  ...
  console.log("SHELVING COLLECTION - MULTI-RETAILER");
  console.log("Focus on shelving units (not hooks)");
  console.log("=".repeat(70));
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log(`SHELVING COLLECTION COMPLETE`);
  console.log(`${"=".repeat(70)}`);
  ...
  if (allProducts.length > 0) {
    ...
    fs.writeFileSync('shelving_collection.json', JSON.stringify(output, null, 2));
    console.log("\n✓ Saved to shelving_collection.json");
...
main().then(count => {
  console.log(`\n✅ Collection complete: ${count} unique shelving products`);
